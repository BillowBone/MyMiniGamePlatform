...
namespace TicTacToe
{
    public partial class Game : XtraForm
    {
        ...
        public Game(Register register)
        {
            register.Dispose();
            InitializeComponent();
            labelControl1.Text = "Actual turn : " + player1;
        ...
        private void ActualizeTurn()
        {
            if (turn == 1 && !finished)
            {
                labelControl1.Text = "Actual turn : " + player2;
                turn = 2;
            }
        ...
        {
            if (my_button.BackColor == Color.FromName("Gainsboro") && !GameFinished(1))
            {
                if (turn == 1)
